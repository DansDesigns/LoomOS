...
# Block systemctl inside chroot — Devuan uses sysvinit but some packages
# call systemctl anyway, causing update-rc.d to fail at line 98.
# This stub silently succeeds so package scripts install without errors.
mkdir -p "$TARGET/usr/local/sbin"
cat > "$TARGET/usr/local/sbin/systemctl" <<'EOF'
#!/bin/sh
exit 0
EOF
chmod +x "$TARGET/usr/local/sbin/systemctl"

# Also stub dbus-launch
mkdir -p "$TARGET/usr/local/bin"
cat > "$TARGET/usr/local/bin/dbus-launch" <<'EOF'
#!/bin/sh
exit 0
EOF
chmod +x "$TARGET/usr/local/bin/dbus-launch"
...
BASE_PKGS=(
    ...
    # Bootloader utilities — grub-common provides update-grub
...
# Ensure python3-venv is installed — it's often missing even when python3 is present
chroot_install python3-venv python3-pip python3-full 2>/dev/null || \
    chroot_install python3-venv python3-pip || true

# Create the venv — with explicit error check
$IN_CHROOT python3 -m venv /opt/loomos/venv --system-site-packages \
    || $IN_CHROOT python3 -m venv /opt/loomos/venv \
    || die "Failed to create Python venv at /opt/loomos/venv"

# Verify pip exists before trying to use it
[[ -f "$TARGET/opt/loomos/venv/bin/pip" ]] \
    || die "Venv created but pip not found — check python3-pip install"

# Upgrade pip first to avoid version issues
$IN_CHROOT /opt/loomos/venv/bin/pip install --quiet --upgrade pip 2>/dev/null || true
...
info "Downloading Ollama binary directly (not using their install.sh — it creates systemd services)..."

# Download tarball directly into chroot — bypass their installer entirely
...
mkdir -p "$TARGET/usr/local/bin"
tar -xzf /tmp/ollama.tgz -C "$TARGET/usr/local/" 2>/dev/null || {
    # Fallback: single binary
    curl -fsSL "https://github.com/ollama/ollama/releases/latest/download/ollama-linux-${OLLAMA_ARCH}" \
        -o "$TARGET/usr/local/bin/ollama" \
        || die "All Ollama download methods failed"
}
rm -f /tmp/ollama.tgz

# Remove any systemd units Ollama tarball may have included
rm -rf "$TARGET/usr/local/lib/systemd" 2>/dev/null || true
rm -f  "$TARGET/etc/systemd/system/ollama.service" 2>/dev/null || true

chmod +x "$TARGET/usr/local/bin/ollama"
[[ -f "$TARGET/usr/local/bin/ollama" ]] || die "Ollama binary not found after install"

# Create ollama user and storage directory
...
# useradd -m creates the home directory — but only if the skeleton exists
# Ensure /etc/skel exists in chroot first
...
# Verify home directory was actually created — useradd -m can fail silently
if [[ ! -d "$TARGET/home/$USERNAME" ]]; then
    warn "useradd -m did not create home dir — creating manually"
    mkdir -p "$TARGET/home/$USERNAME"
    cp -r "$TARGET/etc/skel/." "$TARGET/home/$USERNAME/" 2>/dev/null || true
fi

# Set password — chpasswd inside chroot needs /etc/shadow and PAM to work
# Use direct shadow file manipulation as fallback if chpasswd fails
TEMP_PASS=$(tr -dc 'a-zA-Z0-9' < /dev/urandom | head -c 16 2>/dev/null \
    || echo "LoOmOs-$(date +%s)")

if ! echo "${USERNAME}:${TEMP_PASS}" | $IN_CHROOT chpasswd 2>/dev/null; then
    warn "chpasswd failed — setting password via openssl hash directly"
    # Generate password hash and write it to shadow directly
    PASS_HASH=$(openssl passwd -6 "$TEMP_PASS" 2>/dev/null \
        || python3 -c "import crypt; print(crypt.crypt('$TEMP_PASS', crypt.mksalt(crypt.METHOD_SHA512)))" 2>/dev/null \
        || echo "!")
    if [[ "$PASS_HASH" != "!" ]]; then
        # Update shadow file directly
        $IN_CHROOT bash -c "
            if grep -q '^${USERNAME}:' /etc/shadow; then
                sed -i 's|^${USERNAME}:[^:]*:|${USERNAME}:${PASS_HASH}:|' /etc/shadow
            else
                echo '${USERNAME}:${PASS_HASH}:19000:0:99999:7:::' >> /etc/shadow
            fi
        "
        log "Password set via shadow file"
    else
        warn "Could not set password — use 'passwd $USERNAME' after boot"
        TEMP_PASS="SET-AFTER-BOOT"
    fi
fi

# Store temp password — shown once at end of install
echo "$TEMP_PASS" > /tmp/loomos_temp_pass

# Set correct ownership on home directory
$IN_CHROOT chown -R "${USERNAME}:${USERNAME}" "/home/$USERNAME" 2>/dev/null || \
    chown -R "$(id -u):$(id -g)" "$TARGET/home/$USERNAME" 2>/dev/null || true

warn "Emergency fallback password set — primary login is face + voice passphrase"

# Auto-start X on tty1 — write to home dir which now definitely exists
...
# Final ownership fix on home directory
$IN_CHROOT chown -R "${USERNAME}:${USERNAME}" "/home/$USERNAME" 2>/dev/null || true
...
# =============================================================================
# REMOVE CHROOT STUBS (allow normal operation on real boot)
# =============================================================================
rm -f "$TARGET/usr/sbin/policy-rc.d"
rm -f "$TARGET/usr/local/sbin/systemctl"
rm -f "$TARGET/usr/local/bin/dbus-launch"
